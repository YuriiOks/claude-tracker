// Delegation graph — force-directed map of agents, skills, commands and rules
// Node sizes encode real metrics:
//   agent  → r scales with callsToday  (busier agent = bigger)
//   skill  → r scales with how many agents in this repo could use it
//   command/rule → fixed small (config nodes; not "active")
//   repo   → fixed large (anchor)
const { useState: useStateG, useEffect: useEffectG, useRef: useRefG, useMemo: useMemoG } = React;

const Graph = ({ repos, onOpen }) => {
  const W = 900, H = 540;
  const [selectedRepo, setSelectedRepo] = useStateG(repos[0]?.id);
  const [layers, setLayers] = useStateG({ agents: true, skills: true, commands: true, rules: true });
  const repo = repos.find(r => r.id === selectedRepo) || repos[0];

  // Build node + edge model from repo data + AGENT_META
  const { initialNodes, edges } = useMemoG(() => {
    const ns = [];
    const agentMeta = window.MOCK_DATA.AGENT_META || {};

    // Repo at center, pinned
    ns.push({ id: '__repo__', label: repo.name, kind: 'repo', r: 34, color: repo.accent, fx: W / 2, fy: H / 2 });

    // Skill→agents lookup (token overlap, same heuristic as before)
    ...
    // Size from real file bytes — sqrt-scaled so a 10× larger file is ~3× larger circle
    const SIZES = window.MOCK_DATA.FILE_SIZES || {};
    const sizeFor = (kind, name, fallbackBytes) => {
      const bytes = SIZES[kind]?.[name] ?? fallbackBytes;
      // sqrt(bytes/1000) → ~1.0 for 1KB, ~3.2 for 10KB
    ...
    if (layers.skills) {
      repo.skills.forEach(s => {
        ...
        if (matches.length) {
          // pin skill to its strongest match (first)
  ...
  useEffectG(() => {
    ...
    const step = () => {
      ...
      const restLen = (e) => {
        ...
        if (e.kind === 'entry') return 230;   // commands sit further out
        if (e.kind === 'context') return 210; // rules
  ...
  const didDragRef = useRefG(false);
  const downPosRef = useRefG(null);
            ...
            return (
              <g key={i}>
                ...
                {e.kind === 'delegate' && !active && (
                  <line
                    x1={a.x} y1={a.y} x2={b.x} y2={b.y}
                    stroke="var(--gold)" strokeWidth="1.4" strokeOpacity="0.55"
                ...
                {active && (
                  <line
                    x1={a.x} y1={a.y} x2={b.x} y2={b.y}
                    stroke="var(--cyan)" strokeWidth="2.5" strokeOpacity="0.9"
            ...
            return (
              ...
              >
                ...
                <circle
                  r={n.r}
                  fill={color}
                  fillOpacity={isHovered ? .28 : (isSmall ? .1 : .14)}
                  stroke={color}
                  strokeWidth={isHovered ? 2 : (isSmall ? 1 : 1.5)}
                  style={{ transition: 'fill-opacity .2s, stroke-width .2s' }}
                />
                {isRepo ? (
                  <text textAnchor="middle" dy="5" fontWeight="700" fontSize="14" fill="var(--txt-bright)">
                    {n.label}
                  </text>
                ) : (
                  <>
                    <text textAnchor="middle" dy={isSmall ? '3' : '3.5'} fontSize={isSmall ? '9' : '11'} fontWeight="700" fill={color}>
                      {glyph(n.kind)}
                    </text>
                    <rect
                      x={-(Math.min(labelText.length, 24) * 3.4)}
                      y={n.r + 5}
                      width={Math.min(labelText.length, 24) * 6.8}
                      height={14}
                      rx={3}
                      fill="var(--card)"
                      stroke="var(--brd2)"
                ...
                {/* Activity badge on agents — small numeric pip */}
                {n.kind === 'agent' && n.meta?.callsToday > 0 && !isHovered && (
                  <g transform={`translate(${n.r * 0.7}, ${-n.r * 0.7})`}>
                    ...
                    <text textAnchor="middle" dy="3" fontSize="8" fontWeight="700" fill={color}>
                      {n.meta.callsToday}
                    </text>
                  </g>
                )}
