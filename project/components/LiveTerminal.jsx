...
const LiveTerminal = ({ events, height = 360, showCursor = true, paused = false, hideHeader = false }) => {
  ...
  const [frozen, setFrozen] = React.useState(events);
  React.useEffect(() => {
    ...
    <div className={'term' + (paused ? ' is-paused' : '') + (hideHeader ? ' no-header' : '')} style={{ height }}>
      ...
      <div className="term-body" ref={bodyRef}>
        {list.map((e, i) => {
          ...
          // Stable key: event identity (kind+t+repo) so React reuses DOM and animation only fires once
...
window.eventToLine = eventToLine;
